...
export const OnDemandCategory = ({ navigation, route }) => {
  ...
  const videos = useSelector((state) => state.data.media.onDemand[title])
    ...
    navigation.setOptions({
      ...
      headerRight: () => (
        <ButtonOpacity
          ...
          onPress={() => {
            navigation.navigate('Modals', { screen: 'VideoSettings', params: { categoryId } })
          }}>
  ...
  function renderItem({ item, idx }) {
    const category = mediaCategories[VIDEO_CATEGORIES.ON_DEMAND]?.find((c) => c.id === item.category_id) || {}
    return (
      <OnDemandListItem
        key={item.id}
        item={item}
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.panelInner}>
          ...
          {search.trim() ? (
            ...
            <ScrollView style={styles.scrollView}>{videos?.map((video, idx) => renderItem({ item: video, idx }))}</ScrollView>
          )}
        </View>
      </View>
    </View>
